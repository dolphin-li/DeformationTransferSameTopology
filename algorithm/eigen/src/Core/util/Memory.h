...
template<typename T, bool Align> EIGEN_DEVICE_FUNC inline T* conditional_aligned_new_auto(size_t size)
{
  ...
  internal::check_size_for_overflow<T>(size);
  T *result = reinterpret_cast<T*>(internal::conditional_aligned_malloc<Align>(sizeof(T)*size));
  if(NumTraits<T>::RequireInitialization)
  {
    ...
    EIGEN_CATCH(...)
    {
      internal::conditional_aligned_free<Align>(result);
      EIGEN_THROW;
    }
  }
  return result;
